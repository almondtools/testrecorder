...
public class ContextSnapshot implements Serializable {
    ...
    public Type getThisType() {
        return Optional.ofNullable(setupThis)
            .map(s -> (Type) s.getType())
            .orElse(signature.signature.declaringClass);
    }

    public Type[] getActualArgumentTypes() {
        Type[] actualArgumentTypes = new Type[signature.signature.argumentTypes.length];
        for (int i = 0; i < actualArgumentTypes.length; i++) {
            actualArgumentTypes[i] = Optional.ofNullable(setupArgs[i].getValue())
                .map(a -> (Type) a.getType())
                .orElse(signature.signature.argumentTypes[i]);
        }
