...
public class DefaultArrayAdaptor extends DefaultSetupGenerator<SerializedArray> implements SetupGenerator<SerializedArray> {
	...
	public Computation tryDeserialize(SerializedArray value, SetupGenerators generator) {
		...
		return generator.forVariable(value, value.getResultType(), local -> {
			...
			String arrayLiteral = arrayLiteral(types.getShortName(value.getResultType()), elements);

			statements.add(assignLocalVariableStatement(types.getShortName(value.getResultType()), local.getName(), arrayLiteral));
