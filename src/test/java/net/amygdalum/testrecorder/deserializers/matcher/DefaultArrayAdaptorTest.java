...
public class DefaultArrayAdaptorTest {
	...
	public void testTryDeserializeObjectArray() throws Exception {
		SerializedArray value = new SerializedArray(BigInteger[].class, BigInteger[].class);
		...
		assertThat(result.getStatements(), empty());
		assertThat(result.getValue(), equalTo("arrayContaining(equalTo(new BigInteger(\"0\")), equalTo(new BigInteger(\"8\")), equalTo(new BigInteger(\"15\")))"));
	}
